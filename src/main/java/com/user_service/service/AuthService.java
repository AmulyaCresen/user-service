...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        return new LoginResponse("Login successful", token, user.getRole(), user.getFullName(), user.getEmail(), user.getGender());
    }
    ...
    public Map<String, String> resetPassword(String email, String otp, String newPassword) {
        ...
        try {
            decodedNewPassword = new String(Base64.getDecoder().decode(newPassword), StandardCharsets.UTF_8);
        } catch (IllegalArgumentException e) {
            decodedNewPassword = newPassword;
        }
        String encoded = passwordEncoder.encode(decodedNewPassword);
